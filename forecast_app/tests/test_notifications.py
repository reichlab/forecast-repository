...
class NotificationTestCase(TestCase):
    ...
    def test_email_notification(self):
        # test that UploadFileJob.save() calls send_notification_email() for if UploadFileJob.SUCCESS and
        # UploadFileJob.FAILED cases
        ...
        with patch('forecast_app.notifications.send_notification_email') as send_email_mock:
            ...
            upload_file_job = UploadFileJob.objects.create(user=mo_user, status=UploadFileJob.PENDING)
            upload_file_job.save()
            send_email_mock.assert_not_called()

            # test UploadFileJob.SUCCESS
            send_email_mock.reset_mock()
            upload_file_job = UploadFileJob.objects.create(user=mo_user, status=UploadFileJob.SUCCESS)
